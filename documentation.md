...
const app = express();
const server = new MyMCPServer({ name: "express-server", version: "1.0.0" });
...
const app = new Hono();
const server = new MyMCPServer({ name: "hono-server", version: "1.0.0" });
let transport: SSEServerTransport | null = null;
...
const server = new MyMCPServer({ name: "fastify-server", version: "1.0.0" });
let transport: SSEServerTransport | null = null;
...
const mcpHandler = createMcpHandler(() => new MyMcpServer({ name: "express-server", version: "1.0.0" }));
...
const app = new Hono();
const mcpHandler = createMcpHandler(() => new MyMcpServer({ name: "hono-server", version: "1.0.0" }));
...
const fastify = Fastify();
const mcpHandler = createMcpHandler(() => new MyMcpServer({ name: "fastify-server", version: "1.0.0" }));
...
const mcpHandler = createMcpHandler(() => new MyMcpServer({ name: "edge-server", version: "1.0.0" }));
...
async function start() {
  const server = new CalculatorServer({ name: "calc-server", version: "1.0.0" }, { capabilities: {} });
  new CalcHandlers();
  await server.connect(new StdioServerTransport());
}
...
async function startClient() {
  const client = new AppClient({ name: "calc-client", version: "1.0.0" }, { capabilities: {} });
